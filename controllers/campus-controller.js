...
const createCampus = async (req, res) => {
    try {
        const { 
            campusName, campusCode, address, city, phoneNumber, 
            email, school, isMain, principalName, totalCapacity,
            password // Added password for principal account
        } = req.body;
        ...
        // Check if email is already taken by any staff member
        if (email) {
            const existingStaff = await Staff.findOne({ email: email.toLowerCase() });
            if (existingStaff) {
                return res.status(400).json({
                    success: false,
                    message: "Email is already assigned to another staff member"
                });
            }
        }
        ...
        // Create Principal Staff Account if password is provided
        if (password && email && principalName) {
            const salt = await bcrypt.genSalt(10);
            const hashedPassword = await bcrypt.hash(password, salt);

            const principalStaff = new Staff({
                name: principalName,
                email: email.toLowerCase(),
                password: hashedPassword,
                designation: 'Principal',
                role: 'Admin', // Legacy role for compatibility
                school: school,
                campus: newCampus._id,
                status: 'pending' // Set to pending for main branch approval
            });
        ...
        res.status(201).json({
            success: true,
            message: "Campus and Principal account created successfully",
            campus: newCampus
        });
    } catch (error) {
